...
public class GroupsApi: AdaptableApi {
	...
	public func getUserGroups(tenantId: String, currentPage: Int? = nil, pageSize: Int? = nil, withTotalElements: Bool? = nil, withTotalPages: Bool? = nil) -> AnyPublisher<C8yUserGroupCollection, Swift.Error> {
	...
	/// 	- tenantId 
	///		  Unique identifier of a Cumulocity IoT tenant.
	public func createUserGroup(body: C8yGroup, tenantId: String) -> AnyPublisher<C8yGroup, Swift.Error> {
		...
		return self.session.dataTaskPublisher(for: adapt(builder: builder).build()).tryMap({ element -> Data in
			...
			guard httpResponse.statusCode != 409 else {
				throw Errors.badResponseError(response: httpResponse, reason: "Duplicate – Group name already exists.")
			}
	...
	public func getUserGroup(tenantId: String, groupId: Int) -> AnyPublisher<C8yGroup, Swift.Error> {
		let builder = URLRequestBuilder()
	...
	/// 	- groupId 
	///		  Unique identifier of the user group.
	public func updateUserGroup(body: C8yGroup, tenantId: String, groupId: Int) -> AnyPublisher<C8yGroup, Swift.Error> {
		var requestBody = body
		requestBody.roles = nil
		requestBody.`self` = nil
		requestBody.id = nil
	...
	/// 	- groupId 
	///		  Unique identifier of the user group.
	public func deleteUserGroup(tenantId: String, groupId: Int) -> AnyPublisher<Data, Swift.Error> {
		let builder = URLRequestBuilder()
			.set(resourcePath: "/user/\(tenantId)/groups/\(groupId)")
	...
	public func getUserGroupByName(tenantId: String, groupName: String) -> AnyPublisher<C8yGroup, Swift.Error> {
		let builder = URLRequestBuilder()
	...
	/// 	- withTotalPages 
	///		  When set to `true`, the returned result will contain in the statistics object the total number of pages. Only applicable on [range queries](https://en.wikipedia.org/wiki/Range_query_(database)).
	public func getUserGroups(tenantId: String, userId: String, currentPage: Int? = nil, pageSize: Int? = nil, withTotalElements: Bool? = nil, withTotalPages: Bool? = nil) -> AnyPublisher<C8yGroupReferenceCollection, Swift.Error> {
		var queryItems: [URLQueryItem] = []
		if let parameter = currentPage { queryItems.append(URLQueryItem(name: "currentPage", value: String(parameter))) }
		if let parameter = pageSize { queryItems.append(URLQueryItem(name: "pageSize", value: String(parameter))) }
		if let parameter = withTotalElements { queryItems.append(URLQueryItem(name: "withTotalElements", value: String(parameter))) }
