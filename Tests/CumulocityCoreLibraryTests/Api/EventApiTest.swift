...
public class EventApiTest: XCTestCase {
	
	class TestableEventApi: EventApi {
	...
	public func testGetEventsApiResource() {
		let expectation = XCTestExpectation(description: "ok")
		var cancellables = Set<AnyCancellable>()
		try? TestableEventApi().getEventsApiResource().sink(receiveCompletion: { completion in
			print(completion)
