...
public class DeviceControlApiTest: XCTestCase {
	
	class TestableDeviceControlApi: DeviceControlApi {
	...
	public func testGetDeviceControlApiResource() {
		let expectation = XCTestExpectation(description: "ok")
		var cancellables = Set<AnyCancellable>()
		try? TestableDeviceControlApi().getDeviceControlApiResource().sink(receiveCompletion: { completion in
			print(completion)
