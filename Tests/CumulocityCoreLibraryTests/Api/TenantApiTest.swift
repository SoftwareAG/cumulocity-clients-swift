...
public class TenantApiTest: XCTestCase {
	
	class TestableTenantApi: TenantApi {
	...
	public func testGetTenantsApiResource() {
		let expectation = XCTestExpectation(description: "ok")
		var cancellables = Set<AnyCancellable>()
		try? TestableTenantApi().getTenantsApiResource().sink(receiveCompletion: { completion in
			print(completion)
