...
public class AlarmApiTest: XCTestCase {
	
	class TestableAlarmApi: AlarmApi {
	...
	public func testGetAlarmsApiResource() {
		let expectation = XCTestExpectation(description: "ok")
		var cancellables = Set<AnyCancellable>()
		try? TestableAlarmApi().getAlarmsApiResource().sink(receiveCompletion: { completion in
			print(completion)
