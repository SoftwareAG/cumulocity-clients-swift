...
public class Notification20ApiTest: XCTestCase {
	
	class TestableNotification20Api: Notification20Api {
	...
	public func testGetNotificationApiResource() {
		let expectation = XCTestExpectation(description: "ok")
		var cancellables = Set<AnyCancellable>()
		try? TestableNotification20Api().getNotificationApiResource().sink(receiveCompletion: { completion in
			print(completion)
