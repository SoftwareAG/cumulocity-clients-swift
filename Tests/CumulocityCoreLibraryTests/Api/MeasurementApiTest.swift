...
public class MeasurementApiTest: XCTestCase {
	
	class TestableMeasurementApi: MeasurementApi {
	...
	public func testGetMeasurementApiResource() {
		let expectation = XCTestExpectation(description: "ok")
		var cancellables = Set<AnyCancellable>()
		try? TestableMeasurementApi().getMeasurementApiResource().sink(receiveCompletion: { completion in
			print(completion)
