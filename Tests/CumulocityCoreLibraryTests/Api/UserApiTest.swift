...
public class UserApiTest: XCTestCase {
	
	class TestableUserApi: UserApi {
	...
	public func testGetUserApiResource() {
		let expectation = XCTestExpectation(description: "ok")
		var cancellables = Set<AnyCancellable>()
		try? TestableUserApi().getUserApiResource().sink(receiveCompletion: { completion in
			print(completion)
