...
public class InventoryApiTest: XCTestCase {
	
	class TestableInventoryApi: InventoryApi {
	...
	public func testGetInventoryApiResource() {
		let expectation = XCTestExpectation(description: "ok")
		var cancellables = Set<AnyCancellable>()
		try? TestableInventoryApi().getInventoryApiResource().sink(receiveCompletion: { completion in
